...
import { ArrowLeft, Play, CheckCircle, XCircle, MessageSquare, Calendar, Clock, Loader2 } from "lucide-react";
...
const ConversationPage = () => {
  ...
  const handleStartMeeting = () => {
    ...
    toast({ title: "Meeting Started", description: "Beginning task review process..." });
  };

  const handleTaskAnswer = async (completed: boolean) => {
    setCurrentAnswer(completed);
    setLoading(true);
    const task = meeting!.tasks[currentTaskIndex];

    try {
      const prompt = `
        Task: "${task.title}"
        Status: ${completed ? "Completed" : "Not completed"}
        Generate 3 short follow-up questions to understand context, challenges, and next steps.
      `;
      ...
      const followups = res.choices[0]?.message?.content
        ...
        .filter(Boolean) || ["Please provide more details."];
      ...
      setTaskResponses((prev) => [
        ...prev,
        { taskId: task.id, completed, answers: [] },
      ]);
    } catch (err) {
      console.error(err);
      toast({ title: "Error", description: "Failed to generate follow-ups", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitFollowup = () => {
    ...
    const updatedResponses = taskResponses.map((r) =>
      r.taskId === meeting!.tasks[currentTaskIndex].id
        ? {
            ...r,
            answers: [
              ...r.answers,
              { question: currentFollowups[currentFollowupIndex], answer: currentAnswerText },
            ],
          }
        : r
    );
    ...
    if (currentFollowupIndex < currentFollowups.length - 1) {
      ...
      if (currentTaskIndex < meeting!.tasks.length - 1) {
        ...
        generateAISummary(updatedResponses);
      }
    }
  };
  ...
  const generateAISummary = async (responses: TaskResponse[]) => {
    setLoading(true);
    toast({ title: "Generating Summary", description: "AI is analyzing responses..." });

    try {
      const summary = await generateMeetingSummary(meeting!, responses);
      setAiSummary(summary);
      setShowSummary(true);
      toast({ title: "Summary Generated", description: "Meeting analysis complete!" });
    } catch (err) {
      console.error(err);
  ...
  const progress =
    meetingStarted
      ? ((currentTaskIndex + (currentAnswer !== null ? 0.5 : 0)) / meeting.tasks.length) * 100
      : 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-subtle relative overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              {meetingStarted && (
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-accent/10 text-accent-foreground rounded-full text-xs font-medium">
                  <div className="w-2 h-2 bg-accent rounded-full animate-pulse"></div> In Progress
                </div>
              )}
            </div>
            ...
            <p className="text-muted-foreground mt-1 flex items-center gap-4">
              <span className="flex items-center gap-1">
                <Calendar className="h-4 w-4" /> {new Date(meeting.date).toLocaleDateString()}
              </span>
              <span className="flex items-center gap-1">
            ...
            {meetingStarted && (
              <div className="mt-6 bg-card/50 rounded-xl p-6 backdrop-blur-sm border border-border/30">
      ...
      <div className="relative container mx-auto px-6 py-12">
        {!meetingStarted ? (
          /* Start Meeting Overview */
          <div className="max-w-4xl mx-auto space-y-6">
            <Card className="bg-card/80 backdrop-blur-sm border-border/50">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <MessageSquare className="h-5 w-5 text-primary" /> Meeting Details
                </CardTitle>
              </CardHeader>
            ...
            <Card className="bg-card/80 backdrop-blur-sm border-border/50">
              ...
              <CardContent>
                ...
                <div className="mt-6 text-center">
                  <Button onClick={handleStartMeeting} size="lg" className="bg-primary hover:bg-primary-hover text-primary-foreground px-8">
          ...
          <div className="max-w-4xl mx-auto">
            <Card className="bg-card/80 backdrop-blur-sm border-border/50">
              <CardHeader>
                <CardTitle className="text-center text-primary">Meeting Summary Generated</CardTitle>
              </CardHeader>
              <CardContent>
                <pre className="whitespace-pre-wrap font-sans text-sm leading-relaxed text-card-foreground">{aiSummary}</pre>
                <div className="mt-6 flex justify-center gap-4">
                  <Button variant="secondary" onClick={() => navigate("/")}>Back to Meetings</Button>
                  <Button onClick={() => {
            ...
            <Card className="bg-card/80 backdrop-blur-sm border-border/50">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">{currentTaskIndex + 1} of {meeting.tasks.length}</Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="text-center">
                  ...
                  <div className="flex justify-center gap-4 mb-6">
                    <Button variant={currentAnswer === true ? "default" : "outline"} onClick={() => handleTaskAnswer(true)} className="flex items-center gap-2">
                      <CheckCircle className="h-4 w-4" /> Yes
                    </Button>
                    <Button variant={currentAnswer === false ? "default" : "outline"} onClick={() => handleTaskAnswer(false)} className="flex items-center gap-2">
                ...
                {loading && (
                  <div className="flex items-center justify-center gap-2 text-muted-foreground">
                    <Loader2 className="animate-spin w-5 h-5" /> Loading follow-ups...
                  </div>
                )}
                ...
                {currentAnswer !== null && !loading && (
                  <div className="space-y-4">
                    <Separator />
                    <div>
                      <label className="text-sm font-medium">{currentFollowups[currentFollowupIndex]}</label>
                      <textarea
                    ...
                    <Button onClick={handleSubmitFollowup} disabled={!currentAnswerText.trim()} className="w-full">
                      {currentFollowupIndex < currentFollowups.length - 1
                        ? "Next Question"
                        : currentTaskIndex < meeting.tasks.length - 1
                        ? "Next Task"
                        : "Generate Summary"}
                    </Button>
                  </div>
